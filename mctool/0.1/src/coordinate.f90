...
!-------------------------------------------------------------------!
 ...
 subroutine coo_print_info(vinp, vcoo)
   ...
   do iUnit = 1, nUnit
      ...
      write (*,230) '   Connects two latt.points      : ', vinp%isConn(iUnit)
...
!-------------------------------------------------------------------!
 ...
 subroutine coo_getorder(vcoo, NameAtom_unit)
   ...
   do iSpec = 1, vcoo%nSpec
      vcoo%nOrderSpec_unit(iSpec,1) = iSpec
   enddo
...
!-------------------------------------------------------------------!
 ...
 subroutine coo_getposcar(f_name, nSpec, nAtom, LattVec, NameAtom, &
   ...
   u = io_getunit()
   if (present(force_out)) DatPos%lforce = .true.
   ...
   coo_out   = DatPos%coo
   if (present(force_out)) force_out = DatPos%force

   deallocate (DatPos%NameAtoms, DatPos%nAtomSpec, DatPos%coo)
   if (present(force_out)) deallocate (DatPos%force)
...
!-------------------------------------------------------------------!
 ...
 subroutine coo_writeposcar(f_name, nSpec, nAtom, f_comm, LattVec, &
   ...
   if (present(force_in)) then
      DatPos%lforce = .true.
      DatPos%force  = force_in 
   endif

   call poscar_write(u, DatPos)

   deallocate (DatPos%NameAtoms, DatPos%nAtomSpec, &
