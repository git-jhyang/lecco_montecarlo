...
!integer :: Yii, Yij, Yik
!double precision, dimension(3) :: Yvec1, Yvec2
!double precision :: Yval1, Yval2
...
!-------------------------------------------------------------------!
 ...
 function ene_init(vinp, vcoo) result (iserr)
   ...
   if (pMast) then
      call io_printhead('Energy calculation')
   ...
   case ('AENET')
      if (pMast) then
         call io_printhead('AENET Initialization')
         call inp_read_network(vinp%fname_in, vcoo%nSpec, vcoo%NameAtom, aenet_networks, iserr)
      endif
write (*,*) nrnk, 'net_readed'
      call pp_bcast(iserr)
      if (iserr) then
         if (pMast) call io_printerr('energy initalize','fail to read aenet network files')
         return
      endif

write (*,*) nrnk, 'print err'
      call pp_bcast(aenet_networks, vcoo%nSpec)

      call aenet_init(vcoo%NameAtom, stat)

write (*,*) nrnk, 'aenet init'
      if (stat .ne. CHKOK) then
         if (pMast) call io_printerr('aenet initialize','fail to initialize aenet')
         iserr = .true.
         return
      endif
      loop_spec : do iSpec = 1, vcoo%nSpec
         ...
         if (stat .ne. CHKOK) then
            if (pMast) call io_printerr('aenet initialize - network loading',vcoo%NameAtom(iSpec))
            iserr = .true.
            return
         endif
      enddo loop_spec
write (*,*) nrnk, 'load potential'
      if (pMast) then
!         call aenet_print_info()
         call io_printhead('AENET initialization finished')
      endif
      aenet_isinit = .true.
   end select
write (*,*) nrnk, 'ene_init_fin?'
   call pp_barrier()
write (*,*) nrnk, 'ene_init_fin!'   
   ene_isinit = .true.
...
!-------------------------------------------------------------------!
 ...
 function calc_energy(nSpec, nAtom, LattVec, NameAtom, TypeAtoms, &
   ...
   if (.not. ene_isinit) then
      if (pMast) call io_printerr('energy initialize', method)
      call ene_fin(iserr)
      return
   endif
   ...
   case ('AENET')
      if (.not. aenet_isinit) then
         if (pMast) call io_printerr('energy initialize','AENET is not initialized')
         iserr = .true.
         return
      endif
      if (lforce) then
...
!-------------------------------------------------------------------!
 ...
 subroutine ene_fin(iserr) 
   ...
   if (.not. ene_isinit) then
      if (pMast) call io_printerr('energy finalize','energy is not initialized')
      iserr = .true.
      return
   endif
...
!-------------------------------------------------------------------!
 ...
 subroutine get_energy_aenet(nAtom, LattVec, TypeAtoms, ispbc, coo_in, &
   ...
   real(kind=dp), dimension(3)        :: coo_i
...
write (*,'(2(I5,A),I5,3F12.4)') iAtom+iprc, ' /', nAtom,'  __recv', iprc, coo_frac(:,iAtom+iprc)
      ...
      call pp_barrier()
   enddo loop_atom_2
write (*,*) nrnk, 'coord_bcasting...'
   call pp_bcast(coo_frac, 3, nAtom)
write (*,*) nrnk, 'coord_bcasted'
   if (present(force)) lforce = .true.
write (*,*) nrnk, 'ene calc start'
!if (pMast) then
!do iAtom = 1, nAtom
!   write (*,'(2I4,6F12.5)') iAtom, TypeAtoms(iAtom), coo_frac(:,iAtom), coo_in(:,iAtom)
!enddo
!endif
!stop
...
!write (*,*) nrnk, iAtom, 'lcl load', nnb
      coo_i = coo_frac(:,iAtom)
      call dir_to_cart(LattVec, coo_i)
!write (*,'(" Atom: ",I4," COO: ",3F12.6)') iAtom, coo_i
!do Yii = 1, nnb
!Yvec1 = coo_i - nbcoo(:,Yii)
!Yval1 = sqrt(sum(Yvec1*Yvec1))
!write (*,'(5x," DIST: ",F12.6," COO: ",3F12.6)') Yval1, Yvec1
!enddo

      if (lforce) then
         call aenet_atomic_energy_and_forces(coo_i, TypeAtoms(iAtom), &
                        iAtom, nnb, nbcoo, nbtype, nblist, nAtom, E_i,&
                        force, stat)
      else
         call aenet_atomic_energy(coo_i, TypeAtoms(iAtom), nnb, nbcoo,&
                        nbtype, E_i, stat)
      endif
      E_tot = E_tot + E_i
...
!if (E_i .gt. 1d1) then
! write (*,'(3I5,4F18.6)') iAtom, nAtom, TypeAtoms(iAtom), E_i, coo_i
! do iprc = 1, nnb   
!  write (*,'(7x,2I4,4F18.6)') nblist(iprc), nbtype(iprc), nbdist(iprc),nbcoo(:,iprc)
! enddo
! write (*,*)
!endif
