...
if (pMast) then
   call io_printhead('OPTIMIZATION START')
!   call io_printhead('OPTIMIZATION START',u=u)
endif
...
!write (*,*) '=======================================- ',iOptTry
         ...
         if (ene_iserr) then
            ...
            if (pMast) call io_printerr('main - energy calculation',f_comm)
            main_iserr = .true.
            call finalize()
         endif
...
!-------------------------------------------------------------------!
 ...
 subroutine initialize()
   ...
   vinp = inp_init('OPT')
   if (.not. vinp%pp) call io_printerr('main - input bcast','input is not bcasted')
   ...
   vlat = lat_init(vinp)
   if (.not. vlat%pp) call io_printerr('main - lattice bcast','lattice is not bcasted')
   if (vlat%iserr) return

   vcoo = coo_init(vinp)
   if (.not. vcoo%pp) call io_printerr('main - coord bcast','coord is not bcasted')
   if (vcoo%iserr) return

   vopt = opt_init(vinp, vcoo, vlat)
   if (.not. vopt%pp) call io_printerr('main - optimizer bcast','optimizer is not bcasted')
   ...
   if (pMast) then
      write (f_name,'(A,"_OUT_INIT")') trim(vinp%fname_coo_out)
      write (f_comm,'("INITIAL_STRUCTURE_",ES16.9E2," eV")') ene_tot(1)
      call coo_writeposcar(f_name, vcoo%nSpec, vopt%nAtom, f_comm, vopt%LattVec, &
                           vcoo%NameAtom, vopt%nAtomSpec, .true., vopt%coo)
      call main_calctime()
      write (*,'(2x,"Elapsed time for initialization : ",A14)') time_form(3)
      u_out = io_getunit()
      open (u_out, name='output.log', action='write')
   endif
...
!-------------------------------------------------------------------!
 ...
 subroutine finalize()
   ...
   if (pMast) then
      if (main_iserr) then
         call io_printhead('ERROR - OPTIMIZATION TERMINATED - ERROR')
      else
         write (*,'(2x,"Elapsed Time : ",A13)') time_form(3)
         call io_printhead('OPTIMIZATION FINISHED')
         call io_printhead('OPTIMIZATION FINISHED',u=u_out)
      endif
      close (u_out)
   endif
   call pp_fin()
...
!-------------------------------------------------------------------!
 ...
 subroutine main_printinfo_mig(is_accept)
   ...
   if (is_accept) then
      ...
      write (u_out,'(5x,"iL_MIG= ",I5,"  iL_TRY= ",I5,"  dE= ",ES13.6E2, " eV  dT= ",A14)') &
                iOptMig, iOptTry, ene_tot(3)-ene_tot(2), adjustl(time_form(1))
      ene_tot(2) = ene_tot(3)
      ene_coh(2) = ene_coh(3)
   else
...
!-------------------------------------------------------------------!
 ...
 subroutine main_printinfo_site(is_accept)
   ...
   if (is_accept .or. (iOptMig .ne. 1)) then
      write (*,'(3x,"E_COHENSIVE  = ",F18.6," eV")') ene_coh(2)
      write (*,'(3x,"E_TOTAL      = ",F18.6," eV")') ene_tot(2)
      write (*,'(3x,"dE_SITE      = ",F18.6," eV")') ene_tot(2) - ene_tot(1)
   endif
   ...
   if (is_accept .or. (iOptMig .ne. 1)) then
      ...
      write (u_out,'(2x,"iLS= ",I6,"  ET= ",F18.6,"  EC= ",F18.6,"  dE= ",ES10.3E2,"  T= ",A14)') &
            iOptSite, ene_tot(2), ene_coh(2), ene_tot(2)-ene_tot(1), adjustl(time_form(3))
   else
