...
use lattice,    only :  Var_Latt
 ...
 integer,       dimension(:,:),     allocatable, private :: num2lidx

 logical,                                        private :: opt_isinit = .false.
 logical,                                        private :: opt_iserr  = .false.
 logical,                                        private :: isConnG
 logical,       dimension(:),       allocatable, private :: isConnU
...
!-------------------------------------------------------------------!
 ...
 function opt_init(vinp, vcoo, vlat) result(vopt)
   ...
   if (opt_isinit) then
      if (pMast) call io_printerr('optimizer initialize','optimizer is already initialized')
      vopt%iserr = .true.
      return
   endif
   ...
   nAtoms     = vinp%nAtoms
   allocate (num2lidx(3, nSite))
   num2lidx   = vlat%num2lidx
   nAtoms_max = maxval(nAtoms)
   allocate (isConnU(nUnit))
   isConnG    = vinp%isConn(0)
   isConnU    = vinp%isConn(1:nUnit)
...
!-------------------------------------------------------------------!
 ...
 subroutine opt_fin(vopt)
   ...
   if (.not. opt_isinit) then
      if (pMast) call io_printerr('optimizer finalize','optimizer is not initialized')
      vopt%iserr = .true.
      return
   endif
   ...
   if (allocated (num2lidx)) deallocate (num2lidx)
   if (allocated (isConnU)) deallocate (isConnU)
...
!-------------------------------------------------------------------!
 ...
 subroutine opt_ranwalk(rate_dis, rate_ang, iSite_in, iUnit_in)
   ...
   integer :: iUnit, nAtom, iAt, iS0, iS1, iS2
...
!   write (*,*) '####', iSite_in, iUnit_in

!   if (mod(iSite_in,2) == 0) then
!      iS1 = iSite_in - 1
!      iS2 = iSite_in 
!   else
!      iS1 = iSite_in
...
!      iS0 = iS1
!      nAtom = nAtoms(iUnit)
!      if (.not. isMove(iUnit,mod(iS1,2))) iS0 = iS2
!      do iAt = 1 , nAtom
!         write (*,'(2(A,3F12.6))') '      ',coo_orig(:,iAt,iUnit,iS0), ' / ',coo_ran(:,iAt,iUnit,iS0)
!      enddo
!      write (*,*)
!   enddo
...
!-------------------------------------------------------------------!
 ...
 subroutine opt_bcast(vopt)
   ...
   if (vopt%iserr) return
   if (vopt%pp) then
      if (pMast) call io_printerr('optimizer bcast','optimizer is already bcasted')
      vopt%iserr = .true.
      return
   endif
...
!-------------------------------------------------------------------!
 ...
 subroutine opt_default(vcoo, vopt, nSite_in)
   ...
   vopt%nSite = nSite_in
   if (isConnG) vopt%nAtom = vopt%nAtom/2
   ...
   do iSpec = 1, nSpec
      ...
      do iSpec_Unit = 1, vcoo%nOrderSpec_unit(0,iUnit)
         ...
         enddo
         if (io_isodd(nSite_in)) vopt%nAtomSpec(iSpec) = vopt%nAtomSpec(iSpec) + &
                                   vcoo%nAtomSpec_unit(iSpec_Unit,iUnit)
      enddo
      enddo
...
!-------------------------------------------------------------------!
 ...
 subroutine opt_coo_shift(iSite_in, coo_in, coo_out)
   ...
   integer :: iUnit, iCnt, ixyz, nAtom

   integer, dimension(3,2)   :: lidx
   integer, dimension(3,0:1) :: lidx_pbc

   real(kind=dp), dimension(3) :: DirConn
   real(kind=dp), dimension(3) :: LattLoc
   ...
   if (isConnG) then
      do ixyz = 1, 3
         if ((lidx(ixyz,1) - lidx(ixyz,2)) .gt.  1) lidx_pbc(ixyz,0) = lidx(ixyz,1) 
         if ((lidx(ixyz,1) - lidx(ixyz,2)) .lt. -1) lidx_pbc(ixyz,1) = lidx(ixyz,2) 
      enddo
   endif

!write (*,'(3I4,"  /  ",3I4)') lidx_pbc(:,0), lidx_pbc(:,1)

   if (isConnG) DirConn = opt_coo_shift_get_dir(lidx_pbc)
   iCnt = 0

   LU: do iUnit = 1, nUnit
      if (isConnG .and. (.not. isConnU(iUnit))) iCnt = iCnt + 1
      if (.not. isMove(iUnit,mod(iSite_in,2))) cycle LU
      LattLoc = opt_coo_shift_get_center(iUnit, iCnt, lidx_pbc)
      nAtom   = nAtoms(iUnit)
      coo_out(:,1:nAtom,iUnit) = coo_in(:,1:nAtom,iUnit)
      if (isConnG) call coo_align_v(nAtom, p_zp, DirConn, coo_out(:,1:nAtom,iUnit))
      call coo_trans(nAtom, LattLoc, coo_out(:,1:nAtom,iUnit))
   enddo LU
...
!-------------------------------------------------------------------!
 ...
 function opt_coo_shift_get_dir(lidx) result (DirConn)
   implicit none
   integer,       dimension(3,0:1), intent(in) :: lidx
   real(kind=dp), dimension(3)                 :: DirConn
   ...
   do i = 1, 3
    ...
    case (0, 1, 2)
      DirConn(:) = DirConn(:) + LattVec(:,i)*dble(lidx(i,0) - lidx(i,1))
    end select
   enddo

 end function opt_coo_shift_get_dir

!-------------------------------------------------------------------!

 function opt_coo_shift_get_center(iUnit, iCnt, lidx) result (LattLoc)
   implicit none
   ...
   integer :: ixyz
   logical :: isZ
   logical, dimension(0:1) :: isEvenZ

   isEvenZ(0) = .not. io_isodd(lidx(3,0))
   isEvenZ(1) = .not. io_isodd(lidx(3,1))
   LattLoc(:) = 0d0

   do ixyz = 1, 3
      isZ = .false.
      if (ixyz .eq. 3) isZ = .true.
    select case (LattType)
    case (0) ! Simple lattice
      if (isConnU(iUnit)) then
         LattLoc = LattLoc + LattVec(:,ixyz)*0.5d0*dble(lidx(ixyz,1) + lidx(ixyz,0))
      else
         LattLoc = LattLoc + LattVec(:,ixyz)*dble(lidx(ixyz,(mod(iCnt,2))))
      endif
    case (2) ! face centered lattice
      if (isConnU(iUnit)) then
         LattLoc = LattLoc + LattVec(:,ixyz)*0.25d0*dble(lidx(ixyz,1) + lidx(ixyz,0))
      else
         LattLoc = LattLoc + LattVec(:,ixyz)*0.5d0*dble(lidx(ixyz,(mod(iCnt,2))))
      endif
    case (3) ! hexagonal closed packing lattice
      if (isConnU(iUnit)) then
         LattLoc = LattLoc + LattVec(:,ixyz)*0.5d0*dble(lidx(ixyz,1) + lidx(ixyz,0))
         if ((.not. isZ) .and. isEvenZ(0)) LattLoc = LattLoc + LattVec(:,ixyz)/6d0
         if ((.not. isZ) .and. isEvenZ(1)) LattLoc = LattLoc + LattVec(:,ixyz)/6d0
         if (isZ) LattLoc = LattLoc - LattVec(:,ixyz)*0.25d0*dble(lidx(ixyz,1) + lidx(ixyz,0))
      else
         LattLoc = LattLoc + LattVec(:,ixyz)*dble(lidx(ixyz,mod(iCnt,2)))
         if ((.not. isZ) .and. isEvenZ(mod(iCnt,2))) LattLoc = LattLoc + LattVec(:,ixyz)/3d0
         if (isZ) LattLoc = LattLoc - LattVec(:,ixyz)*0.5d0*dble(lidx(ixyz,mod(iCnt,2)))
      endif
    end select
   enddo

 end function opt_coo_shift_get_center
...
!-------------------------------------------------------------------!
 ...
 subroutine opt_coo_gather(vcoo, vopt, iSite_in)
   ...
   stack = iSite_in - 1
   if (isConnG) stack = floor(stack*0.5d0)*2
      ...
      loop_unit : do iUnit = 1, nUnit
         loop_order : do iSpecUnit = 1, vcoo%nOrderSpec_unit(0,iUnit)
            ...
            do while (.true.)
               ...
               if (isConnG .and. (mod(iSite_in,2) .ne. ii)) cycle
...
!-------------------------------------------------------------------!
 ...
 function opt_lidx_get(iSite_in) result (lidx_out)
   ...
   if (.not. isConnG) then
      lidx_out(:,1) = num2lidx(:,iSite_in)
      lidx_out(:,2) = lidx_out(:,1)
   elseif (io_isodd(iSite_in)) then
      lidx_out(:,1) = num2lidx(:,iSite_in)
      lidx_out(:,2) = num2lidx(:,iSite_in + 1)
   else
      lidx_out(:,1) = num2lidx(:,iSite_in - 1)
      lidx_out(:,2) = num2lidx(:,iSite_in)
   endif
