...
!-------------------------------------------------------------------!
 ...
 subroutine opt_coo_init(vinp, vcoo)
   implicit none
   type(Var_Input), intent(in) :: vinp
   type(Var_Coord), intent(in) :: vcoo

   type(Var_Opt) :: vopt_loc

   integer :: iSite1, iSite2, iUnit, rUnit, iAtom, nAtom
   integer :: cnt_find, iConn, nConn, rConn, iL
   logical :: iserr, isExit

!   real(kind=dp) :: rad, dis, val
   real(kind=dp) :: rate_dis, rate_ang!, rate_tol
   real(kind=dp), dimension(3)                  :: ene_tot, ene_coh
!   real(kind=dp), dimension(3)                  :: vec
!   real(kind=dp), dimension(nUnit)              :: radius
!   real(kind=dp), dimension(3,nUnit,nSite)      :: centroid
   real(kind=dp), dimension(3,nAtoms_max,nUnit,0:3) :: coo_save
   real(kind=dp), dimension(3,nAtoms_max,nUnit,0:1) :: coo_move

   if (pMast) then
      vopt_loc%LattVec(:,1) = LattVec(:,1)*vinp%nx
      vopt_loc%LattVec(:,2) = LattVec(:,2)*vinp%ny
      vopt_loc%LattVec(:,3) = LattVec(:,3)*vinp%nz
      allocate (coo_orig (3, nAtoms_max, nUnit, nSite), &
                coo_ran  (3, nAtoms_max, nUnit, nSite), &
                coo_shift(3, nAtoms_max, nUnit, nSite))
      nConn = 0
      if (isConnect(0)) nConn = 1
      ene_coh(1) = 0d0
   endif

   call pp_bcast(vopt_loc%LattVec, 3, 3)
   call pp_bcast(nConn)
  
   do iSite1 = 1, nSite, nConn+1
      call opt_default(vcoo, vopt_loc, iSite1+nConn)
      if (pMast) then
         do iConn = 0, nConn
            coo_save(:,:,:,iConn)   = vcoo%coo_unit
            coo_save(:,:,:,iConn+2) = vcoo%coo_unit
            call opt_coo_shift(iSite1+iConn, coo_save(:,:,:,iConn), &
                               coo_shift(:,:,:,iSite1+iConn))
         enddo
         do iSite2 = 1, iSite1+nConn
            call opt_coo_gather(vcoo, vopt_loc, iSite2)
         enddo
         rate_dis = 1.5d0
         rate_ang = 0.5d0
         isExit   = .false.
      endif
      call pp_bcast(vopt_loc%TypeAtoms, vopt_loc%nAtom)
      call pp_bcast(vopt_loc%coo, 3, vopt_loc%nAtom)
      iserr = calc_energy(nSpec, vopt_loc%nAtom, vopt_loc%LattVec, &
                          vcoo%NameAtom, vopt_loc%TypeAtoms, vinp%ispbc, &
                          vopt_loc%coo, ene_coh(2), ene_tot(2))
      loop_init : do iL = 1, vinp%nOptTry
         if (pMast) then
            do iConn = 0, nConn
               coo_move(:,:,:,iConn) = coo_save(:,:,:,iConn)
               do iUnit = 1, nUnit
                  if (.not. isMove(iUnit,mod(iSite1+iConn,2))) cycle
                  call coo_rot_v(nAtoms(iUnit), p_zp, coo_move(:,1:nAtoms(iUnit), &
                                 iUnit,iConn), twpi*grnd())
               enddo
               call opt_coo_ranwalk(iSite1+iConn, rate_dis, rate_ang, &
                                    coo_move(:,:,:,iConn), coo_ran(:,:,:,iSite1+iConn))
               call opt_coo_shift(iSite1+iConn, coo_ran(:,:,:,iSite1+iConn), &
                                  coo_shift(:,:,:,iSite1+iConn))
               call opt_coo_gather(vcoo, vopt_loc, iSite1+iConn)
            enddo
         endif
         call pp_bcast(vopt_loc%TypeAtoms, vopt_loc%nAtom)
         call pp_bcast(vopt_loc%coo, 3, vopt_loc%nAtom)
         iserr = calc_energy(nSpec, vopt_loc%nAtom, vopt_loc%LattVec, &
                             vcoo%NameAtom, vopt_loc%TypeAtoms, vinp%ispbc, &
                             vopt_loc%coo, ene_coh(3), ene_tot(3))
         if (pMast) then
            if (ene_coh(3) .lt. ene_coh(2)) then
               ene_coh(2) = ene_coh(3)
               do iConn = 0, nConn
                  coo_save(:,:,:,iConn+2) = coo_move(:,:,:,iConn)
               enddo
            endif
            cnt_find = 0
         endif
      ...
      loop_find : do
         if (pMast) then
            if (ene_coh(2) .lt. ene_coh(1)) then
               write (*,*) '  ',iSite1, ene_coh(1)
               isExit = .true.
            else
               cnt_find = cnt_find + 1
               if (mod(cnt_find,200) .eq. 0) write (*,*) '//',iSite1, ene_coh(1), ene_coh(3)-ene_coh(2)
               if (mod(cnt_find,500) .eq. 0) rate_dis = rate_dis + 1d0
               if (rate_dis .gt. 3d0) then
            ...
            if (isExit) then
               do iConn = 0, nConn
                  coo_orig(:,:,:,iSite1+iConn) = coo_save(:,:,:,iConn+2)
               enddo
               ene_coh(1) = ene_coh(2)
            endif
         endif
         call pp_bcast(isExit)
         if (isExit) exit loop_find

         if (pMast) then
            do
               rUnit = floor(grnd()*nUnit)+1
               if (isConnect(rUnit)) cycle
               rConn = floor(grnd()*(nConn+1))
               if (isMove(rUnit,mod(iSite1+rConn,2))) exit
            enddo
            coo_move(:,:,:,0) = coo_save(:,:,:,rConn+2)
            call coo_rot_v(nAtoms(rUnit), p_zp, coo_move(:,1:nAtoms(iUnit), &
                           rUnit,0), twpi*rate_ang*grnd())
            call opt_coo_ranwalk(iSite1+rConn, rate_dis, rate_ang, &
                                 coo_move(:,:,:,0), coo_ran(:,:,:,iSite1+rConn))
            do iUnit = 1, nUnit
               if (iUnit .eq. rUnit) cycle
               if (.not. isMove(iUnit,mod(iSite1+rConn,2))) cycle
               coo_ran(:,:,iUnit,iSite1+rConn) = coo_save(:,:,iUnit,rConn+2)
            enddo
            call opt_coo_shift(iSite1+rConn, coo_ran(:,:,:,iSite1+rConn), &
                               coo_shift(:,:,:,iSite1+rConn))
            call opt_coo_gather(vcoo, vopt_loc, iSite1+rConn)
         endif
         call pp_bcast(vopt_loc%TypeAtoms, vopt_loc%nAtom)
         call pp_bcast(vopt_loc%coo, 3, vopt_loc%nAtom)
         iserr = calc_energy(nSpec, vopt_loc%nAtom, vopt_loc%LattVec, &
...
!         do iUnit = 1, nUnit
!            if (.not. isMove(iUnit,mod(iSite1,2))) cycle
!            nAtom = nAtoms(iUnit)
!            val   = rate_ang*twpi*grnd()
!            call coo_rot_v(nAtom, p_zp, coo_tmp(:,1:nAtom,iUnit), val)
!         enddo
