...
if (E_i .gt. 1d1) then
! write (*,'(4F15.6)') aenet_pot(type_i)%E_scale, E_i_arr(1), &
! aenet_pot(type_i)%E_shift, aenet_pot(type_i)%E_atom
write (*,'("Type: ",I3," / E: ",F22.15," / COO: ",3F10.4)') type_i, E_i, coo_i
do ii = 1, nsf
if (abs(sfval(ii)) .gt. 1d2) write (*,'(" SFN: ",I6," / SFVAL: ",F22.8)') ii, sfval(ii)
enddo
endif
