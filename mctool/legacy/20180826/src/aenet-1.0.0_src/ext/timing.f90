...
contains
  ...
  subroutine tng_timing(msg, silent)
    ...
    call system_clock(count=cnt, count_rate=cnt_rate, count_max=cnt_max)
    ...
    if (tng_t_now .lt. tng_t_prev) tng_t_prev = tng_t_prev - dble(cnt_max)/dble(cnt_rate)
    if (tng_t_now .lt. tng_t_lastlog) tng_t_lastlog = tng_t_lastlog - dble(cnt_max)/dble(cnt_rate)
