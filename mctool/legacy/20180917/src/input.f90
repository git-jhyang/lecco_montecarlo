...
!       isConnect(n)    : 'L.CONNECT'   : if 'true', connect unit 'n' 
!                                         connects two lattice points
!       isConnect(0)        : not given     : 'true' if one of isConnect(n) is 'true' 
!
!       = character =
...
! common
   ...
   logical                                             :: iserr = .false.
   logical                                             :: lseed
   logical,                dimension(:),   allocatable :: isConnect
...
! generator
   ...
   integer,       dimension(2)   :: nGridTrans
   real(kind=dp)                 :: r_cut
   real(kind=dp)                 :: AngRotG
   real(kind=dp), dimension(3)   :: AngRotL
   real(kind=dp), dimension(2,2) :: DisTrans
 end type Var_Input
...
!                       isConnect(0) and .not. isConnect(n), duplicate 
...
!-------------------------------------------------------------------!
 ...
 function inp_init(mode) result(vinp)
   ...
   if (inp_isinit) then
      if (pMast) call mcio_printerr('input initialize','input is already initialized')
      vinp%iserr = .true.
      return
   endif
   ...
   if (pMast) then
      ...
      case ('OPT')
         isfile = mcio_assert_file('opt.in')
         inp_iserr = .not. isfile
         if (isfile) vinp = inp_read_opt('opt.in')
      case ('GEN')
         isfile = mcio_assert_file('gen.in')
         inp_iserr = .not. isfile
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_fin(vinp)
   ...
   if (.not. inp_isinit) then
      if (pMast) call mcio_printerr('input finalize','input is not initialized')
      vinp%iserr = .true.
      return
   endif
   ...
   if (allocated (vinp%isConnect)) deallocate (vinp%isConnect)
...
!-------------------------------------------------------------------!
 ...
 function inp_read_opt(fname) result(vinp)
   ...
   allocate (vinp%isConnect(0:nUnit))
   ...
   call inp_read_val(u, 'L.CONNECT',    vinp%isConnect(1:nUnit), nUnit)
   ...
   vinp%isConnect(0) = .false.
   do iUnit = 1, nUnit
      if (vinp%isConnect(iUnit)) vinp%isConnect(0) = .true.
   enddo

   call inp_read_val(u, 'ENE.METHOD', vinp%method)
   call inp_read_val(u, 'LATT.TYPE',  vinp%LattType, .true.)
   ...
   select case (trim(adjustl(vinp%LattType)))
   case default 
      vinp%LattType = 'S'
   ...
   if (vinp%isConnect(0) .and. (mod(vinp%nSite,2) .eq. 1)) vinp%nSite = vinp%nSite + 1
   if (vinp%isConnect(0)) call inp_extend(vinp)
...
!-------------------------------------------------------------------!
 ...
 function inp_read_gen(fname) result(vinp)
   ...
   call inp_read_val(u, 'NUM.UNITS', nUnit)
   if (nUnit .ne. 2) then
      call mcio_printerr('input parameter - NUM.UNITS','3 or more units are not supported yet')
      return
   endif
   vinp%nUnit = nUnit
   ...
   call inp_read_val(u, 'DIST.CUTOFF.MIN',  vinp%r_cut)
   ...
   call inp_read_val(u, 'TRANS.DIS.Y',  vinp%DisTrans(:,1), 2)
   call inp_read_val(u, 'TRANS.DIS.Z',  vinp%DisTrans(:,2), 2)
   call inp_read_val(u, 'TRANS.GRID.Y', vinp%nGridTrans(1))
   call inp_read_val(u, 'TRANS.GRID.Z', vinp%nGridTrans(2))
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_network(fname_in, nSpec, NameAtom, networks, iserr)
   ...
   integer                :: iSpec1, iSpec2, u, ierr, ii, idx
   ...
   call mcio_findline(u, 'AENET.NETWORKS', line, ierr)
   do iSpec1 = 1, nSpec
      read (u, '(A)') line
   ...
   loop_spec_1 : do iSpec1 = 1, nSpec
      ...
      if (networks(iSpec1) .eq. 'NONE') then
         call mcio_printerr('aenet network not found with "AENET.NETWORKS"', NameAtom(iSpec1))
         iserr = .true.
      endif
   enddo loop_spec_1
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_bcast(vinp)
   ...
   if (vinp%iserr) return
   if (vinp%pp) then
      if (pMast) call mcio_printerr('input bcast','input is already bcasted')
      vinp%iserr = .true.
      return
   endif
   ...
   if (.not. allocated (vinp%isConnect)) allocate (vinp%isConnect(0:vinp%nUnit))
   call pp_bcast(vinp%isConnect, vinp%nUnit+1)
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_printinfo_opt(vinp)
   ...
   case('BC', 'BCL', 'BCC')
      LattType_long = 'Base Centered Lattice'
   case('FC', 'FCL', 'FCC')
      LattType_long = 'Face Centered Lattice'
   case('HCP', 'CP')
      LattType_long = 'Hexagonal Closed Packing Lattice'
   end select

   call mcio_printhead('Input parameters')
   ...
   write (*,201) '   Lattice type                  : ', trim(adjustl(LattType_long))
   ...
   write (*,202) '   Connection between latt.point : ', vinp%isConnect(0)
   write (*,*)     
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_printinfo_gen(vinp)
   ...
   write (*,202) '   Minimum distance btw. atoms   : ', vinp%r_cut
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_extend(vinp)
   ...
   logical,                dimension(:),   allocatable :: isConnect_ext(:)
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_fnames(u, chr_in, fnames, nUnit)
   ...
   character(LEN=LINELEN) :: line
   integer                :: iUnit, ierr1, ierr2, isfile

   isfile = .true.

   call mcio_findline(u, chr_in, line, ierr1)
   ierr2 = 0
   if (ierr1 .ne. CHKOK) goto 100
   do iUnit = 1, nUnit
        ! loop for get filenames
      read(u, '(A)', iostat = ierr2, end = 100) line
      write (fnames(iUnit),'(A)') trim(adjustl(line))
      isfile = isfile .and. mcio_assert_file(fnames(iUnit))
   enddo
   if (.not. isfile) inp_iserr = .true.
   return

100 continue
   if ((ierr2 .le. CHKEOF) .or. (ierr1 .eq. CHKEOF)) write (*,*) 'touched end of file'

   inp_iserr = .true.
   call mcio_printerr('input reading', chr_in)
...
100 continue
   if ((ierr2 .le. CHKEOF) .or. (ierr1 .eq. CHKEOF)) write (*,*) 'touched end of file'
   inp_iserr = .true.
   call mcio_printerr('input reading',chr_in, line)
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_val_i1(u, chr_in, int_out, skip_err_in) 
   implicit none
   integer,           intent(in)    :: u
   character(LEN=*),  intent(in)    :: chr_in
   integer,           intent(inout) :: int_out
   ...
   call mcio_findline(u, chr_in, line, ierr)
   if (ierr .eq. CHKOK) then
      read (line, *, err = 100) int_out
   elseif (ierr .eq. CHKERR) then
      goto 100  
   endif
   return       
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_val_in(u, chr_in, ints_out, n, skip_err_in)
   implicit none
   integer,                        intent(in)    :: u
   integer,                        intent(in)    :: n
   character(LEN=*),               intent(in)    :: chr_in
   integer,          dimension(n), intent(inout) :: ints_out
   logical,          optional,     intent(in)    :: skip_err_in
   ...
   call mcio_findline(u, chr_in, line, ierr)
   if (ierr .eq. CHKOK) then
      read (line, *, err = 100) ints_out(1:n)
   elseif (ierr .eq. CHKERR) then
      goto 100
   endif      
   return
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_val_r1(u, chr_in, real_out, skip_err_in)
   implicit none
   integer,           intent(in)    :: u
   character(LEN=*),  intent(in)    :: chr_in
   real(kind=dp),     intent(inout) :: real_out
   logical, optional, intent(in)    :: skip_err_in
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_val_rn(u, chr_in, reals_out, n, skip_err_in)
   implicit none
   integer,                     intent(in)    :: u
   integer,                     intent(in)    :: n
   character(LEN=*),            intent(in)    :: chr_in
   real(kind=dp), dimension(n), intent(inout) :: reals_out
   logical,       optional,     intent(in)    :: skip_err_in
   ...
   call mcio_findline(u, chr_in, line, ierr)
   if (ierr .eq. CHKOK) then
      read (line, *, err = 100) reals_out(1:n)
   elseif (ierr .eq. CHKERR) then
      goto 100
   endif      
   return
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_val_c1(u, chr_in, chr_out, skip_err_in)
   implicit none
   integer,           intent(in)    :: u
   character(LEN=*),  intent(in)    :: chr_in
   character(LEN=*),  intent(inout) :: chr_out
   logical, optional, intent(in)    :: skip_err_in
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_val_cn(u, chr_in, chrs_out, n, skip_err_in)
   implicit none
   integer,                        intent(in)    :: u
   integer,                        intent(in)    :: n
   character(LEN=*),               intent(in)    :: chr_in
   character(LEN=*), dimension(n), intent(inout) :: chrs_out
   logical,          optional,     intent(in)    :: skip_err_in
   ...
   call mcio_findline(u, chr_in, line, ierr)
   if (ierr .eq. CHKOK) then
      read (line, *, err = 100) chrs_out(1:n)
      return
   elseif (ierr .eq. CHKERR) then
      goto 100
   endif      
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_val_l1(u, chr_in, log_out, skip_err_in)
   implicit none
   integer,           intent(in)    :: u
   character(LEN=*),  intent(in)    :: chr_in
   logical,           intent(inout) :: log_out
   logical, optional, intent(in)    :: skip_err_in
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_val_ln(u, chr_in, logs_out, n, skip_err_in)
   implicit none
   integer,                        intent(in)    :: u
   integer,                        intent(in)    :: n
   character(LEN=*),               intent(in)    :: chr_in
   logical,          dimension(n), intent(inout) :: logs_out
   logical,          optional,     intent(in)    :: skip_err_in
   ...
   call mcio_findline(u, chr_in, line, ierr)
   if (ierr .eq. CHKOK) then
      read (line, *, err = 100) logs_out(1:n)
      return
   elseif (ierr .eq. CHKERR) then
      goto 100
   endif      
