...
do while (.not. vgen%isfin)
   call gen_load_coo(vgen)
   if (vgen%iserr) then
      call mcio_printerr('generator failed','error occured')
      main_iserr = .true.
      exit
   endif
...
!-------------------------------------------------------------------!
 ...
 subroutine initialize()
   ...
   call pp_init()
   if (nprc .ne. 1) then
      call mcio_printerr('parallelization','program designed for serial run')
      return
   endif
   ...
   if (pMast) then
      ...
      u = mcio_getunit()
      open (u, name='gen.log', action='write')
   endif
...
!   if (.not. vopt%iserr) call opt_fin(vgen)
   ...
   if (pMast) then
      if (main_iserr) then
         call mcio_printhead('ERROR - GENERATOR TERMINATED - ERROR')
      else
         write (*,'(2x,"Elapsed Time : ",A13)') time_form(3)
         call mcio_printhead('GENERATION FINISHED')
         call mcio_printhead('GENERATION FINISHED',u=u)
      endif
      close (u)
   endif
   call pp_fin()
