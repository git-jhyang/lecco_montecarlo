...
contains
  ...
  subroutine stp_normalize(stp, deriv)
    ...
    do isf = 1, stp%nsf
       ...
       s = stp%sfval_cov(isf) - shift*shift
       if (s < 0d0) then
          s = abs(s) !!jhyang!!
          if (s .ge. 1.0d-10) write (*,*) 'JHYANG debug - negative scaling factor ',s
       endif
       s = sqrt(s)
! write (*,*) stp%sfval_avg(isf), stp%sfval_cov(isf), s, shift
       if (s <= 1.0d-10) then
...
! write (*,'(4ES14.3E2)') sfval(isf), scale, s, shift !!jhyang!!
