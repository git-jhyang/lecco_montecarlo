...
program train
  ...
  if (ppMaster) then
     ! devide structures into test and train set: !!jhyang!!
     if (inp%trn_readtest) then !!jhyang!!
        call decide_testing_set_jhyang(ts%nStrucs, inp%trn_ntestset, &
                                nTrain, nTest, isTest, restarted)
     else !!jhyang!!
...
! if (ppMaster) write(*,*) MAE_trn, SSE_trn, nTrain, MAE_tst, SSE_tst, nTest, ts_trn%scale
...
if (E_i .ne. E_i) then
write (*,*) 'sss', E_i, net(itype), nsf, sfval(1:nsf)
endif
       E_nn = E_nn + E_i
  ...
  !--------------------------------------------------------------------!
  ! edited by jhyang
  ! set first n files as testing set

  subroutine decide_testing_set_jhyang(nFiles, nTestFiles, nTrain, &
                                nTest, isTest, restarted)

    implicit none

    integer,                    intent(in)  :: nFiles
    integer,                    intent(in)  :: nTestFiles
    integer,                    intent(out) :: nTrain
    integer,                    intent(out) :: nTest
    logical, dimension(nFiles), intent(out) :: isTest
    logical,                    intent(out) :: restarted
    ...
    nTrain = nFiles - nTestFiles
    nTest  = nTestFiles

    isTest(:) = .false.
    isTest(1:nTestFiles) = .true.

    ! re-initialize random number generator, in case it is used for
    ! something else afterwards
    call random_reinit()

  end subroutine decide_testing_set_jhyang
