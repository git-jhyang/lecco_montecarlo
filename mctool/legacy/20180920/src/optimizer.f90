...
!-------------------------------------------------------------------!
 ...
 subroutine opt_coo_shift(iSite_in, coo_in, coo_out)
   ...
   LU1: do iUnit = 1, nUnit
      if (.not. isConnect(iUnit)) icnt = icnt + 1
      if (.not. isMove(iUnit,mod(iSite_in,2))) cycle LU1
      LXYZ: do ixyz = 1, 3
         select case (LattType)
         case ('S')
            if (.not. isConnect(0)) then
               LattLoc(:,iUnit) = LattLoc(:,iUnit) + LattVec(:,ixyz) * dble(lidx(ixyz,1)-1)
            elseif (isConnect(iUnit)) then
               LattLoc(:,iUnit) = LattLoc(:,iUnit) + LattVec(:,ixyz) * 0.5d0 * &
                                  dble(lidx_pbc(ixyz,1) + lidx_pbc(ixyz,0) - 2)
            else
               LattLoc(:,iUnit) = LattLoc(:,iUnit) + LattVec(:,iXYZ) * &
                                  dble(lidx_pbc(ixyz,(mod(icnt,2))) - 1)
            endif
         case ('FC')
            if (.not. isConnect(0)) then
               LattLoc(:,iUnit) = LattLoc(:,iUnit) + LattVec(:,ixyz) * 0.5d0 * dble(lidx(ixyz,1)-1)
            elseif (isConnect(iUnit)) then
               LattLoc(:,iUnit) = LattLoc(:,iUnit) + LattVec(:,ixyz) * 0.25d0 * &
                                  dble(lidx_pbc(ixyz,1) + lidx_pbc(ixyz,0) - 2)
            else
               LattLoc(:,iUnit) = LattLoc(:,iUnit) + LattVec(:,iXYZ) * 0.5d0 * &
                                  dble(lidx_pbc(ixyz,(mod(icnt,2))) - 1)
