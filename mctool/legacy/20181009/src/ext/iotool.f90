...
interface io_linesplit
   module procedure     io_linesplit_i,       io_linesplit_l,       &
                        io_linesplit_r,       io_linesplit_d,       &
                        io_linesplit_c
...
!--------------------------------------------------------------------------!
 ...
 subroutine io_checkdim(line, n, CHK, nMax)
   ...
   if (present(nMat)) iCnt = nMax

 end subroutine io_checkdim

!--------------------------------------------------------------------------!

 subroutine io_linesplit_i(line_in, n, line_out, CHK)
   implicit none
   character(LEN=*),      intent(in)  :: line_in
   integer,               intent(in)  :: n
...
100 continue
   call io_printerr('read error - io_linesplit',&
...
!--------------------------------------------------------------------------!
 ...
 subroutine io_linesplit_l(line_in, n, line_out, CHK)
   implicit none
   character(LEN=*),      intent(in)  :: line_in
   integer,               intent(in)  :: n
...
!--------------------------------------------------------------------------!
 ...
 subroutine io_linesplit_r(line_in, n, line_out, CHK)
   implicit none
   character(LEN=*),   intent(in)  :: line_in
   integer,            intent(in)  :: n
...
!--------------------------------------------------------------------------!
 ...
 subroutine io_linesplit_d(line_in, n, line_out, CHK)
   implicit none
   character(LEN=*),            intent(in)  :: line_in
   integer,                     intent(in)  :: n
...
!--------------------------------------------------------------------------!
 ...
 subroutine io_linesplit_c(line_in, n, line_out, CHK)
   implicit none
   character(LEN=*),               intent(in)  :: line_in
   integer,                        intent(in)  :: n
...
100 continue
   call io_printerr('read error - io_linesplit',&
        trim(adjustl(line_val))//' is not CHARACTER')
   CHK = CHKERR
