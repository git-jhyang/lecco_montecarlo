...
!-------------------------------------------------------------------!
 ...
 function opt_init(vinp, vcoo, vlat) result(vopt)
   ...
   isConnG = vinp%isConnect(0)
   isConnU = vinp%isConnect(1:nUnit)
...
!-------------------------------------------------------------------!
 ...
 function opt_coo_shift_get_center(iUnit, iCnt, lidx) result (LattLoc)
   ...
   integer :: i
   logical, dimension(0:1) :: isOddZ

   isOddZ(0) = io_isodd(lidx(3,0))
   isOddZ(1) = io_isodd(lidx(3,1))

   do i = 1, 3
    select case (LattType)
    case (0)
      if (.not. isConnG) then
         LattLoc = LattLoc + LattVec(:,i)*dble(lidx(i,1))
      elseif (isConnU(iUnit)) then
         LattLoc = LattLoc + LattVec(:,i)*0.5d0*dble(lidx(i,1) + lidx(i,0))
      else
         LattLoc = LattLoc + LattVec(:,i)*dble(lidx(i,(mod(iCnt,2))))
      endif
    case (2)
      if (.not. isConnG) then
         LattLoc = LattLoc + LattVec(:,i)*0.5d0*dble(lidx(i,1))
      elseif (isConnU(iUnit)) then
         LattLoc = LattLoc + LattVec(:,i)*0.25d0*dble(lidx(i,1) + lidx(i,0))
      else
         LattLoc = LattLoc + LattVec(:,i)*0.5d0*dble(lidx(i,(mod(iCnt,2))))
      endif
    case (3)
      if (.not. isConnG) then
         LattLoc = LattLoc + LattVec(:,i)

x, y벡터만 1/3씩 이동해서 센터잡기.      
    end select
   enddo
