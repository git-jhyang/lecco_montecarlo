...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_network(fname_in, nSpec, NameAtom, networks, iserr)
   ...
   integer                :: iSpec1, iSpec2, u, iCHK, ii, idx
   ...
   call io_findline(u, 'AENET.NETWORKS', line, iCHK)
   do iSpec1 = 1, nSpec
      read (u, '(A)') line
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_fnames(u, chr_in, fnames, nUnit)
   ...
   do iUnit = 1, nUnit
      ...
      call io_checkdim(line, 1, iCnt, CHK)
      if (CHK .ne. CHKOK) then
         call io_printerr('input error - '//chr_in,'line is empty')
         goto 100
      endif
      call io_linesplit(line, 1, fnames(iUnit))
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_vecs(u, chr_in, vecs, nUnit)
   ...
   do iUnit = 1, nUnit
      read(u, '(A)', iostat=iCHK2, end=100) line
      call io_checkdim(line, 3, iCnt, CHK)
      if (CHK .ne. CHKOK) then
         call io_printerr('input error - '//chr_in,'need 3 values')
         goto 100
      endif
      call io_linestrip(line, 3, vecs(:,iUnit))
   enddo
   return
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_val_i1(u, chr_in, val_out, skip_err_in) 
   ...
   call io_findline(u, chr_in, line, iCHK)
   if (iCHK .eq. CHKOK) then
      call io_checkdim(line, 1, iCHK)
      if (iCHK .ne. CHKOK) goto 100
      call io_linesplit(line, 1, val_out)
   elseif (iCHK .eq. CHKERR) then
      goto 100  
   endif
   return       
...
!-------------------------------------------------------------------!
 ...
 subroutine inp_read_val_in(u, chr_in, val_out, n, skip_err_in)
   ...
   call io_findline(u, chr_in, line, iCHK)
   if (iCHK .eq. CHKOK) then
      call io_checkdim(line, n, iCHK)
      if (iCHK .ne. CHKOK) goto 100
      call io_linesplit(line, n, val_out)
   elseif (iCHK .eq. CHKERR) then
      goto 100
   endif      
   return
